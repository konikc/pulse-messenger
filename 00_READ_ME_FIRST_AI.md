...
- [~] Task 3 — Test the auth flow in the browser (sign-in → onboarding → app). Use the `agent-browser` skill.
      **BLOCKED in this environment:** Neon is NOT connected in this fresh clone, so `DATABASE_URL`,
      `NEON_AUTH_BASE_URL`, and `NEON_AUTH_COOKIE_SECRET` are absent. `lib/auth/server.ts` throws
      "Neon Auth environment variables are not configured" at import when they're missing, so the
      sign-in route cannot boot and a green `pnpm build` also requires them. To finish this task:
      connect the **Neon** integration, set `NEON_AUTH_COOKIE_SECRET` (32+ chars via
      `openssl rand -base64 32`), then run `pnpm build` and browser-test `/auth/sign-in`.
- [x] Task 4 — Write `DEPLOY.md` self-host tutorial (Beget/generic VPS: Node + pm2 + nginx + Let's Encrypt) and push to the branch.

### 5a. Note for the next AI (2026-08-02 handoff)

- This branch was re-cloned in a **broken pre-fix state** (Vercel Blob/Analytics, serverless driver,
  25 stray duplicate files in the repo root). Tasks 1 & 2 were **re-applied** here from the reference
  archive: stray root files deleted, media routes back on local storage, `package.json` fixed, and
  `pnpm install` run clean (serverless + analytics removed; `pg` / `swr` / `@livekit/components-react`
  added). `DEPLOY.md` was written (Task 4).
- The only remaining item is the **live auth browser test (Task 3)**, which needs the Neon env vars
  above. Everything else (converged source + DEPLOY.md) is ready to commit/push.
